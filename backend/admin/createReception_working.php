...
try {
    ...
    error_log("createReception_working.php - Incoming data: " . json_encode($data));
    ...
    if (!isset($data->username) || !isset($data->password) || !isset($data->full_name)) {
        ...
        error_log("createReception_working.php - Validation error: " . json_encode($error_response));
    ...
    if ($check_stmt->rowCount() > 0) {
        ...
        error_log("createReception_working.php - Username exists error: " . json_encode($error_response));
        echo json_encode($error_response);
        exit();
    }
    ...
    if ($stmt->execute()) {
        ...
        error_log("createReception_working.php - Success response: " . json_encode($success_response));
        
        echo json_encode($success_response);
    } else {
        http_response_code(500);
        $error_response = array("error" => "Failed to create reception account");
        error_log("createReception_working.php - Database error: " . json_encode($error_response));
        echo json_encode($error_response);
    }
} catch (PDOException $e) {
    error_log("Database error in createReception_working.php: " . $e->getMessage());
    http_response_code(500);
    $error_response = array("error" => "Database error: " . $e->getMessage());
    error_log("createReception_working.php - PDO error response: " . json_encode($error_response));
    echo json_encode($error_response);
} catch (Exception $e) {
    error_log("General error in createReception_working.php: " . $e->getMessage());
    http_response_code(500);
    $error_response = array("error" => "Server error: " . $e->getMessage());
    error_log("createReception_working.php - General error response: " . json_encode($error_response));
    echo json_encode($error_response);
}
?>
