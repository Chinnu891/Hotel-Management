...
echo "=== UPDATING TO REQUESTED ROOM CATEGORIES ===\n\n";
...
try {
    ...
    echo "Current room types:\n";
    $stmt = $db->query("SELECT id, name, base_price FROM room_types ORDER BY id");
    $current_types = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    foreach ($current_types as $type) {
        echo "- ID {$type['id']}: {$type['name']} (\${$type['base_price']})\n";
    }
    echo "\n";
    
    // Update room types to requested categories
    echo "Updating room types to Deluxe, Executive, Suite...\n";
    ...
    echo "\nThe admin website should now be able to fetch rooms successfully!\n";
