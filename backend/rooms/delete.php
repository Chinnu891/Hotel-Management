...
try {
    ...
    // If room has active bookings, return warning (frontend will show popup)
    if ($active_bookings > 0) {
        http_response_code(400);
        echo json_encode(array(
            "error" => "Room has active bookings",
            "warning" => "This room has " . $active_bookings . " active booking(s). Are you sure you want to delete it?",
    ...
    // Delete room (no active bookings) - use room_number as primary key
    ...
    if ($delete_stmt->execute()) {
        ...
        try {
            ...
            $log_stmt->bindValue(":details", "Deleted room: " . $room_number);
            $log_stmt->bindValue(":ip", $_SERVER['REMOTE_ADDR']);
            $log_stmt->execute();
        } catch (Exception $log_error) {
            // Log error but don't fail the room deletion
            error_log("Failed to log room deletion: " . $log_error->getMessage());
        }
        ...
        error_log("Sending delete success response: " . json_encode($response));
