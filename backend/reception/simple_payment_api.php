...
try {
    ...
    if ($action === 'pay_due_amount') {
        ...
        try {
            ...
            // Validate amount - convert to float for proper comparison
            $remainingAmount = floatval($booking['remaining_amount']);
            $paymentAmount = floatval($amount);
            
            error_log("Payment validation: amount=$paymentAmount, remaining=$remainingAmount");
            
            if ($paymentAmount > $remainingAmount) {
                throw new Exception("Payment amount (₹$paymentAmount) cannot exceed remaining amount (₹$remainingAmount)");
            }
